...
export const SERVICES_SCRIPT = `    function renderGatewayApiProfile() {
    ...
    function renderGatewayChatPlatformProfile() {
      ...
      const mobileSharedToken = getEnvironmentValue(profile.environment, 'MOBILE_SHARED_TOKEN');
      document.getElementById('gatewayChatMobileSharedToken').value = isRedactedSecretValue(mobileSharedToken) ? '' : mobileSharedToken;
      document.getElementById('gatewayChatMobileSharedToken').placeholder = isRedactedSecretValue(mobileSharedToken) ? 'Stored; reveal values to inspect' : '';
      document.getElementById('gatewayChatMobileSharedUserId').value = getEnvironmentValue(profile.environment, 'MOBILE_SHARED_USER_ID');
